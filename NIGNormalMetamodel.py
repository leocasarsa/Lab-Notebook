...
class NIGNormalMetamodel(MyMetamodel):
    ...
    def predict_confidence(self, colno, numsamples=None):
        ...
        value = self.get_predictive_params()['loc'][colno]
    ...
    def simulate(self, colnos, constraints=[], numpredictions=1):
        ...
        simulation = np.array([])
        for icol in colnos:
            simulation = np.append(simulation, stats.t.rvs(df[icol], loc=loc[icol], scale=scale[icol], size=numpredictions))
    ...
    def logpdf(self, targets, constraints):
        ...
        for i in np.arange(len(targets)):
            colno = targets[0][i]
            value = targets[1][i]
            logpdf_out += np.log(stats.t.pdf(value, df[colno], loc=loc[colno], scale=scale[colno]))
    ...
    def get_predictive_params(self):
        ...
        Predictive posterior is Student-t distribution.

        See:  http://www.cs.ubc.ca/~murphyk/Papers/bayesGauss.pdf
        '''
    ...
    def get_posterior_hypers(self):
        ...
        Returns a dict mapping parameter names to a list of parameters.
        Posterior distribution is Normal-Inverse-Gamma.
        ...
        # Mean and sum of squares important for posterior update
